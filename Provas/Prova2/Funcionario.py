...
class Funcionario:
    ...
    def grava_funcionario(self, nomeArq):
        '''Pega um arquivo qualquer e grava na ultima linha dele, o funcionario indicado'''
        string = f"{self.cod}|{self.nome}|{self.cpf}|{self.data_nascimento}|{self.salario}|".encode()
        arq = open(nomeArq, "ab")
        arq.write(string)
        arq.write("\n".encode())
        arq.close()

    def le_Linha(self, linha):
        '''Le uma linha de dado arquivo e salva nos atributos da classe'''
        linha = str(linha)
        aux = linha.split("|")
        self.cod = aux[0][2:]
        self.nome = aux[1]
